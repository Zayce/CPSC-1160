...
void insert1(int*& arr, int key, int j) {
    if (key > arr[j] || j < 0) {
        arr[j + 1] = key; // if the key is greater than the j index, then immediately to the right of it will be the key
        return;
    } else {
        arr[j + 1] = arr[j]; //shifts to the right
        insert1(arr, key, j - 1); //Decrements to the left to see which one index position has the value be less than the key
    }
}
